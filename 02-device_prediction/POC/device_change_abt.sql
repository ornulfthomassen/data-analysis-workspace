...
# *********************************************************************
# BUILD ABT´s FOR DEVICE change PREDICTION
# Output table: `tnn-consumer-common-nx.DataScience.device_prediction_abt_bal`
# *********************************************************************

#Find all device first use for each EXISTING subscription wher we have device usage info
CREATE OR REPLACE TABLE
  `tnn-consumer-common-nx.DataScience.subscription_hist_tmp` AS
  WITH 
  HIST_1 AS(
SELECT 
m.main_number_sk,
...
active_subs as (
  ...
  LEFT JOIN `tnn-consumer-common-nx-gold.Dimensions.subscription_master_history` m 
  ON CAST(s.subscription_id AS NUMERIC) = m.subscription_id
  where period_month_date = DATE_SUB(DATE_ADD(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL 1 MONTH), INTERVAL 1 DAY)
   and s.device_type IN ('PDA','CLAMSHELL','SMARTPHONE','BLOCK','SLIDER','RUGGED')
  and RAND() < 0.4
)
select a.* from HIST_1 a
...
CREATE OR REPLACE TABLE
  `tnn-consumer-common-nx.DataScience.device_change_hist_tmp` AS
WITH
#3.1 Rank devices and only keep devices that have first use less than 2 years after release
  ...
  FROM `tnn-consumer-common-nx.DataScience.subscription_hist_tmp` a
  GROUP BY ALL ),
...
WITH
  ...
  FROM
    `tnn-consumer-common-nx.DataScience.device_change_hist_tmp`
  WHERE
...
CREATE OR REPLACE TABLE `tnn-consumer-common-nx.DataScience.device_change_abt_tmp` AS
WITH
...
CASE WHEN s.device_producername NOT IN ( 'APPLE', 'SAMSUNG', 'DORO', 'HUAWEI', 'NOKIA', 'MOTOROLA', 'SONY', 'GOOGLE', 'ONEPLUS', 'XIAOMI', 'KAMMUNICA POLSKA', 'CATERPILLAR', 'SHANDONG KAER ELECTRIC', 'TCL COMMUNICATION', 'ALCATEL', 'ASUS', 'SONYERICSSON', 'JABLOCOM', 'LG', 'NOTHING TECHNOLOGY LIMITED') THEN 1 ELSE 0 END AS device_producername_OTHER_TOP_20,
  ...
  # AGG: 
  MAX(CASE WHEN DATE_DIFF( DATE_TRUNC(s.period_month_date, MONTH), DATE_TRUNC(dc.device_first_use_date, MONTH), DAY) > 365 AND dtarget.device_first_use_date IS NOT NULL THEN 1 ELSE 0 END) AS device_target,
  MIN(dc.device_first_use_date) AS current_device_first_use_date,
  MIN(DATE_DIFF( DATE_TRUNC(s.period_month_date, MONTH), DATE_TRUNC(dc.device_first_use_date, MONTH), DAY)) AS current_device_days,
  ...
  EXTRACT(MONTH FROM dc.device_first_use_date) AS current_purchase_month_nr
FROM
  `tnn-consumer-common-nx.DataScience.subscription_hist_tmp` AS s
LEFT JOIN
...
LEFT JOIN
  `tnn-consumer-common-nx`.`DataScience`.`main_stat_tmp` AS dtarget
ON
  s.main_number_sk = dtarget.main_number_sk
  AND dtarget.device_first_use_date BETWEEN DATE_ADD(DATE_TRUNC(s.period_month_date, MONTH), INTERVAL 1 MONTH)
  AND DATE_ADD(DATE_TRUNC(s.period_month_date, MONTH), INTERVAL 3 MONTH)
JOIN
...
WHERE
  s.period_month_date BETWEEN DATE_ADD(CURRENT_DATE(), INTERVAL -3 YEAR)
...
# Balance the training data, non events needs to be undersampled
CREATE OR REPLACE TABLE `tnn-consumer-common-nx.DataScience.device_change_abt`
AS
SELECT * 
FROM `tnn-consumer-common-nx.DataScience.device_change_abt_tmp`
WHERE DEVICE_TARGET=1
UNION ALL
SELECT *
FROM `tnn-consumer-common-nx.DataScience.device_change_abt_tmp`
WHERE DEVICE_TARGET=0
AND RAND() < 0.02;

#drop table  `tnn-consumer-common-nx.DataScience.subscription_hist_tmp`;
#drop table `tnn-consumer-common-nx.DataScience.device_change_hist_tmp`;
...
drop table `tnn-consumer-common-nx.DataScience.device_change_abt_tmp`;
